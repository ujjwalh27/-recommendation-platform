# Video Intelligence Engine: Human Validation Report

## Video Asset Details
* **Video ID**: `video_ci_1785491770`
* **Duration**: `34.90 seconds`

---

## 👁️ Visual Timeline & Scenes
- **Scene [0.0s - 5.8s]**: Scene keyframe 0 displaying visual elements: {'description': 'Lamps being lit by the person', 'type': 'lighted lamps'}, {'description': "Offering
- **Scene [5.8s - 11.6s]**: Scene keyframe 1 displaying visual elements: {'description': 'Lamps being lit by the person', 'type': 'lighted lamps'}, {'description': "Offering
- **Scene [11.6s - 17.4s]**: Scene keyframe 2 displaying visual elements: {'description': 'Lamps being lit by the person', 'type': 'lighted lamps'}, {'description': "Offering
- **Scene [17.4s - 34.9s]**: Scene keyframe 3 displaying visual elements: {'description': 'Lamps being lit by the person', 'type': 'lighted lamps'}, {'description': "Offering

---

## 🎙️ Audio Transcript (Speech-to-Text)
> I'm going to show you how to make a video on the screen of this video, and I'll see you in the next video. Thank you for watching.

---

## 🧠 Semantic Understanding (Generated Metadata)
* **Title**: Devotional Worship Ceremony
* **Summary**: The sequence of frames depicts a person performing devotional rituals at an ornately decorated shrine, likely within a temple or religious setting. The individual is seen lighting lamps in front of the deity's idol adorned with vibrant flowers and offerings.
* **Category**: Devotion | **Subcategory**: Religious Worship Ceremony
* **Primary Topic**: Worship Rituals
* **Secondary Topics**: Deity Worship, Offerings
* **Language**: English
* **Mood**: Spiritual and Devotional | **Emotion**: Serious, Reverent
* **Target Audience**: Individuals interested in religious practices or spiritual ceremonies.
* **Reasoning Explanation**: The visual elements such as the deity's idol adorned with flowers, lamps being lit, and offerings indicate a devotional worship ceremony. The audio transcript suggests that this video is part of a series on making videos related to devotionals, adding context for viewers interested in creating similar content.

---

## 📊 Secondary Modality Evidence
* **OCR Screen Detections**:
  - *No text blocks scanned.*
* **Visual Object Detections (YOLO/CLIP)**:
  - `person` (Confidence: 0.65)
  - `bowl` (Confidence: 0.51)
  - `cake` (Confidence: 0.48)
  - `vase` (Confidence: 0.39)
  - `Temple, shrine, or devotional place` (Confidence: 0.76)
  - `Stage with music performance` (Confidence: 0.09)
* **Human Action Detections (VideoMAE)**:
  - *No actions classified.*
* **Audio Set Sound Events (AST)**:
  - `Music` (Confidence: 0.71)
  - `Drum` (Confidence: 0.05)

---

## 📈 Provenance & Confidence Evaluation
| Field Name | Confidence Score | Evidence Sources | Algorithmic Reason / Justification |
| :--- | :--- | :--- | :--- |
| **CATEGORY** | 0.85 | Foundation Model, Vision | Vision model detected temple/altar scenes. |
| **SUBCATEGORY** | 0.80 | Foundation Model, Vision | Subcategory derived from category reasoning. Vision model detected temple/altar scenes. |
| **TITLE** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **SUMMARY** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **PRIMARY_TOPIC** | 0.70 | Foundation Model | Inferred semantic category from keyframes. |
| **LANGUAGE** | 0.99 | Foundation Model, Speech | Verified by Whisper speech transcription service language logs. |
| **MOOD** | 0.70 | Foundation Model | Visual mood interpretation. |
| **EMOTION** | 0.70 | Foundation Model | Visual mood interpretation. |
| **ACTIVITIES** | 0.65 | Foundation Model | No corroboration found for list items in secondary actions sensor logs. |
| **OBJECTS** | 0.82 | Foundation Model, YOLO/CLIP | Verified 1/2 elements against YOLO/CLIP modality detections. |

---

## ✍️ Human Auditor Review Feedback
*Please fill out this block to track semantic alignment performance:*

* **Title Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Summary Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Topic & Tag Relevance**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Category Classification**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect

### Review Comments & Notes
```text
[Enter feedback details, discrepancies, or notes here]
```

**Auditor Name**: \_\_\_\_\_\_\_\_\_\_\_\_  
**Date Evaluated**: \_\_\_\_\_\_\_\_\_\_\_\_
