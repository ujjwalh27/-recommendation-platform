# Video Intelligence Engine: Human Validation Report

## Video Asset Details
* **Video ID**: `daiv_s2_10_592504894769465090`
* **Duration**: `9.00 seconds`

---

## 👁️ Visual Timeline & Scenes
- **Scene [0.0s - 1.5s]**: Scene keyframe 0 displaying visual elements: {'object1': 'Deity statue on a decorated platform', 'description': 'Central focus of the ceremony, s
- **Scene [1.5s - 3.0s]**: Scene keyframe 1 displaying visual elements: {'object1': 'Deity statue on a decorated platform', 'description': 'Central focus of the ceremony, s
- **Scene [3.0s - 4.5s]**: Scene keyframe 2 displaying visual elements: {'object1': 'Deity statue on a decorated platform', 'description': 'Central focus of the ceremony, s
- **Scene [4.5s - 9.0s]**: Scene keyframe 3 displaying visual elements: {'object1': 'Deity statue on a decorated platform', 'description': 'Central focus of the ceremony, s

---

## 🎙️ Audio Transcript (Speech-to-Text)
> Thank you for watching.

---

## 🧠 Semantic Understanding (Generated Metadata)
* **Title**: Devotional Ceremony in an Indian Temple
* **Summary**: The sequence captures individuals participating in a devotional ceremony within what appears to be a temple or shrine. The setting includes vibrant floral arrangements around a central deity statue, and various ritualistic objects are present on a decorated platform.
* **Category**: Devotion | **Subcategory**: Religious Ceremony
* **Primary Topic**: A religious devotion involving multiple participants in traditional attire performing rituals.
* **Secondary Topics**: Traditional Indian Attire, Deity Worship
* **Language**: English
* **Mood**: Solemn and Devotional | **Emotion**: Serious, Devotional
* **Target Audience**: Religious followers, People interested in Indian culture
* **Reasoning Explanation**: The video captures a traditional religious ceremony with participants dressed in vibrant attire performing rituals around a deity statue. The setting is indicative of an Indian temple environment where such ceremonies are conducted.

---

## 📊 Secondary Modality Evidence
* **OCR Screen Detections**:
  - *No text blocks scanned.*
* **Visual Object Detections (YOLO/CLIP)**:
  - `person` (Confidence: 0.71)
  - `cake` (Confidence: 0.42)
  - `dining table` (Confidence: 0.40)
  - `Stage with music performance` (Confidence: 0.71)
  - `Temple, shrine, or devotional place` (Confidence: 0.27)
* **Human Action Detections (VideoMAE)**:
  - *No actions classified.*
* **Audio Set Sound Events (AST)**:
  - `Squeal` (Confidence: 0.45)
  - `Jingle bell` (Confidence: 0.11)
  - `Bell` (Confidence: 0.07)

---

## 📈 Provenance & Confidence Evaluation
| Field Name | Confidence Score | Evidence Sources | Algorithmic Reason / Justification |
| :--- | :--- | :--- | :--- |
| **CATEGORY** | 0.85 | Foundation Model, Vision | Vision model detected temple/altar scenes. |
| **SUBCATEGORY** | 0.80 | Foundation Model, Vision | Subcategory derived from category reasoning. Vision model detected temple/altar scenes. |
| **TITLE** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **SUMMARY** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **PRIMARY_TOPIC** | 0.70 | Foundation Model | Inferred semantic category from keyframes. |
| **LANGUAGE** | 0.99 | Foundation Model, Speech | Verified by Whisper speech transcription service language logs. |
| **MOOD** | 0.70 | Foundation Model | Visual mood interpretation. |
| **EMOTION** | 0.70 | Foundation Model | Visual mood interpretation. |
| **ACTIVITIES** | 0.65 | Foundation Model | No corroboration found for list items in secondary actions sensor logs. |
| **OBJECTS** | 0.65 | Foundation Model | No corroboration found for list items in secondary vision sensor logs. |

---

## ✍️ Human Auditor Review Feedback
*Please fill out this block to track semantic alignment performance:*

* **Title Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Summary Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Topic & Tag Relevance**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Category Classification**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect

### Review Comments & Notes
```text
[Enter feedback details, discrepancies, or notes here]
```

**Auditor Name**: \_\_\_\_\_\_\_\_\_\_\_\_  
**Date Evaluated**: \_\_\_\_\_\_\_\_\_\_\_\_
