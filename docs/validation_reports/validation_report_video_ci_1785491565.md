# Video Intelligence Engine: Human Validation Report

## Video Asset Details
* **Video ID**: `video_ci_1785491565`
* **Duration**: `16.17 seconds`

---

## 👁️ Visual Timeline & Scenes
- **Scene [0.0s - 2.7s]**: Scene keyframe 0 displaying visual elements: Ornately decorated shrine with flowers and fabric, Musical instrument being played by the person lea
- **Scene [2.7s - 5.4s]**: Scene keyframe 1 displaying visual elements: Ornately decorated shrine with flowers and fabric, Musical instrument being played by the person lea
- **Scene [5.4s - 8.1s]**: Scene keyframe 2 displaying visual elements: Ornately decorated shrine with flowers and fabric, Musical instrument being played by the person lea
- **Scene [8.1s - 16.2s]**: Scene keyframe 3 displaying visual elements: Ornately decorated shrine with flowers and fabric, Musical instrument being played by the person lea

---

## 🎙️ Audio Transcript (Speech-to-Text)
> All right, let's go to the other side of the world.

---

## 🧠 Semantic Understanding (Generated Metadata)
* **Title**: Devotional Procession in a Religious Context
* **Summary**: The video captures a vibrant devotional procession involving individuals dressed in traditional attire, carrying an ornately decorated shrine. The setting appears to be outside of a temple or religious site with onlookers present.
* **Category**: Religion | **Subcategory**: Devotion
* **Primary Topic**: A religious ceremonial event featuring a deity's idol being carried through the streets in a procession.
* **Secondary Topics**: Traditional attire, Ornate decorations
* **Language**: English
* **Mood**: Spiritual and Devotional | **Emotion**: ['Serious', 'Respectful']
* **Target Audience**: Devotees of the deity being carried, People interested in religious ceremonies
* **Reasoning Explanation**: The video showcases a traditional devotional procession with individuals dressed in vibrant, ceremonial attire carrying an idol adorned with flowers and fabric. The setting suggests it is taking place outside a temple or similar site, indicating its cultural significance.

---

## 📊 Secondary Modality Evidence
* **OCR Screen Detections**:
  - `0` (Confidence: 0.37)
* **Visual Object Detections (YOLO/CLIP)**:
  - `person` (Confidence: 0.64)
  - `Temple, shrine, or devotional place` (Confidence: 0.46)
  - `Person talking to camerin reel or video` (Confidence: 0.33)
  - `Stage with music performance` (Confidence: 0.17)
* **Human Action Detections (VideoMAE)**:
  - *No actions classified.*
* **Audio Set Sound Events (AST)**:
  - `Music` (Confidence: 0.90)

---

## 📈 Provenance & Confidence Evaluation
| Field Name | Confidence Score | Evidence Sources | Algorithmic Reason / Justification |
| :--- | :--- | :--- | :--- |
| **CATEGORY** | 0.70 | Foundation Model | Primary VLM classification. No matching secondary sensor logs to confirm. |
| **SUBCATEGORY** | 0.65 | Foundation Model | Subcategory derived from category reasoning. Primary VLM classification. No matching secondary sensor logs to confirm. |
| **TITLE** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **SUMMARY** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **PRIMARY_TOPIC** | 0.70 | Foundation Model | Inferred semantic category from keyframes. |
| **LANGUAGE** | 0.99 | Foundation Model, Speech | Verified by Whisper speech transcription service language logs. |
| **MOOD** | 0.70 | Foundation Model | Visual mood interpretation. |
| **EMOTION** | 0.70 | Foundation Model | Visual mood interpretation. |
| **ACTIVITIES** | 0.65 | Foundation Model | No corroboration found for list items in secondary actions sensor logs. |
| **OBJECTS** | 0.82 | Foundation Model, YOLO/CLIP | Verified 1/2 elements against YOLO/CLIP modality detections. |

---

## ✍️ Human Auditor Review Feedback
*Please fill out this block to track semantic alignment performance:*

* **Title Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Summary Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Topic & Tag Relevance**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Category Classification**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect

### Review Comments & Notes
```text
[Enter feedback details, discrepancies, or notes here]
```

**Auditor Name**: \_\_\_\_\_\_\_\_\_\_\_\_  
**Date Evaluated**: \_\_\_\_\_\_\_\_\_\_\_\_
