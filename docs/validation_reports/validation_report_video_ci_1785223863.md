# Video Intelligence Engine: Human Validation Report

## Video Asset Details
* **Video ID**: `video_ci_1785223863`
* **Duration**: `10.80 seconds`

---

## 👁️ Visual Timeline & Scenes
- **Scene [0.0s - 1.8s]**: Scene keyframe 0 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [1.8s - 3.6s]**: Scene keyframe 1 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [3.6s - 5.4s]**: Scene keyframe 2 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [5.4s - 7.2s]**: Scene keyframe 3 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [7.2s - 9.0s]**: Scene keyframe 4 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [9.0s - 10.8s]**: Scene keyframe 5 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 
- **Scene [10.8s - 10.8s]**: Scene keyframe 6 displaying visual elements: {'name': 'Offerings on the altar'}, {'name': 'Fire in a vessel being held by the person'}, {'name': 

---

## 🎙️ Audio Transcript (Speech-to-Text)
> If you're happy, if you're on the right side, you're not on the left side, and if you can't be happy, you are on the wrong side.

---

## 🧠 Semantic Understanding (Generated Metadata)
* **Title**: Devotional Ceremony in a Temple
* **Summary**: The video captures a devotional ceremony taking place at what appears to be a temple or shrine. A person dressed in traditional attire performs rituals involving offerings and fire.
* **Category**: Devotion | **Subcategory**: Religious Rituals
* **Primary Topic**: A religious offering ritual being performed by an individual
* **Secondary Topics**: Offerings, Fire ceremony
* **Language**: English
* **Mood**: Spiritual, Devotional | **Emotion**: [{'description': 'The individual appears focused and reverent during the ceremony.'}]
* **Target Audience**: Religious followers, People interested in cultural rituals
* **Reasoning Explanation**: Based on visual cues such as traditional attire, religious items, offerings, and fire ceremony, it is clear that this video depicts a devotional ritual taking place at a temple or shrine.

---

## 📊 Secondary Modality Evidence
* **OCR Screen Detections**:
  - *No text blocks scanned.*
* **Visual Object Detections (YOLO/CLIP)**:
  - `bowl` (Confidence: 0.54)
  - `banana` (Confidence: 0.54)
  - `person` (Confidence: 0.49)
  - `cup` (Confidence: 0.39)
  - `Temple, shrine, or devotional place` (Confidence: 0.96)
* **Human Action Detections (VideoMAE)**:
  - *No actions classified.*
* **Audio Set Sound Events (AST)**:
  - `Jingle bell` (Confidence: 0.36)
  - `Music` (Confidence: 0.35)
  - `Tambourine` (Confidence: 0.10)

---

## 📈 Provenance & Confidence Evaluation
| Field Name | Confidence Score | Evidence Sources | Algorithmic Reason / Justification |
| :--- | :--- | :--- | :--- |
| **CATEGORY** | 0.85 | Foundation Model, Vision | Vision model detected temple/altar scenes. |
| **SUBCATEGORY** | 0.80 | Foundation Model, Vision | Subcategory derived from category reasoning. Vision model detected temple/altar scenes. |
| **TITLE** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **SUMMARY** | 0.75 | Foundation Model | Inferred context from visual representation. |
| **PRIMARY_TOPIC** | 0.70 | Foundation Model | Inferred semantic category from keyframes. |
| **LANGUAGE** | 0.99 | Foundation Model, Speech | Verified by Whisper speech transcription service language logs. |
| **MOOD** | 0.70 | Foundation Model | Visual mood interpretation. |
| **EMOTION** | 0.70 | Foundation Model | Visual mood interpretation. |
| **ACTIVITIES** | 0.65 | Foundation Model | No corroboration found for list items in secondary actions sensor logs. |
| **OBJECTS** | 0.82 | Foundation Model, YOLO/CLIP | Verified 2/4 elements against YOLO/CLIP modality detections. |

---

## ✍️ Human Auditor Review Feedback
*Please fill out this block to track semantic alignment performance:*

* **Title Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Summary Accuracy**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Topic & Tag Relevance**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect
* **Category Classification**: [ ] Correct  |  [ ] Partially Correct  |  [ ] Incorrect

### Review Comments & Notes
```text
[Enter feedback details, discrepancies, or notes here]
```

**Auditor Name**: \_\_\_\_\_\_\_\_\_\_\_\_  
**Date Evaluated**: \_\_\_\_\_\_\_\_\_\_\_\_
